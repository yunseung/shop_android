...
//    private static int thisPage;
    ...
    public PartnerLayout(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs);
        initView(context, null);
    }
    ...
    protected void onFinishInflate() {
        ...
        mViewPagerCardList = (InfiniteViewPager)findViewById(R.id.viewPager_partner);
...
//        mAdapterGSXBrand = new AdapterGSXBrand(itemList);
        try {
            TextView textTitle = (TextView) findViewById(R.id.txt_navi_partner_title);
        ...
        final PartnersPagerAdapter basicPageAdapter = new PartnersPagerAdapter(mContext, item.subContentList);
        final InfinitePagerAdapter wrappedAdapter = new InfinitePagerAdapter(
                new PartnersPagerAdapter(mContext, item.subContentList));

        if (item.subContentList.size() > PartnersPagerAdapter.PARTNER_MENU_PAGE_ADAPTER_SIZE) {
...
//                    thisPage = virtualPosition;
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            mViewPagerCardList.setAccessibilityDelegate(new View.AccessibilityDelegate() {
                @Override
