...
/**
 * 하단 상품정보 (방송상품, 편성표, 내일TV, 단품 등에서 공통으로 사용) V2
 *
 */
@SuppressLint("NewApi")
public class RenewalPriceInfoBottomViewHolder extends TLineBaseViewHolder {
     ...
     */
    public RenewalPriceInfoBottomViewHolder(View itemView) {
        super(itemView);
            ...
            public void onClick(View view) {
                ...
                if( ApptimizeCommand.ABINFO_VALUE.contains(ApptimizeExpManager.SCHEDULE)){
                    ...
                    try {
                        ...
                        eventProperties.put(AMPEnum.AMP_ACTION_NAME, AMPEnum.AMP_CLICK_SCHEDULE_ALARM);

                        if(tvLiveBanner != null) {
                            eventProperties.put(AMPEnum.AMP_PRD_CODE, tvLiveBanner.rPrdId);
                            eventProperties.put(AMPEnum.AMP_PRD_NAME, tvLiveBanner.rProductName);
                            eventProperties.put(AMPEnum.AMP_AB_INFO, ApptimizeCommand.ABINFO_VALUE);
                        }

                        AMPAction.sendAmpEventProperties(AMPEnum.AMP_CLICK_SCHEDULE_ALARM,eventProperties);
                    } catch (JSONException exception){

                    }
                    //앱티마이즈를 위한 코드
                    Apptimize.track("Click-편성표-방송알림");
                }
            }
        });
            ...
            if (footer_text_1.length() > 0) {
                ...
                for (ImageBadge badge : tvLiveBanner.rAllBenefit) {
                    ...
                    if (index >= 0 && !TextUtils.isEmpty(badge.type)) {
                        try {
                            sourcingStringBuilder.setSpan(new ForegroundColorSpan(Color.parseColor("#" + badge.type)),
    ...
    //                  새로 그리는 타이밍에서 타이밍 이슈가 발생해서 두번 줄바꿈 하는 문제가 발생하는 것으로 보임 이에 \n이 있을 경우 줄바꿈 로직 수행하지 않음.
                            ...
                            try {
                                if (STYLETYPE_BOLD.equals(tempTvLiveBanner.rSource.styleType)) {
                                    sourcingStringBuilder2.setSpan(new StyleSpan(Typeface.BOLD), 0,
                                            tempTvLiveBanner.rSource.text.length(),
                                            Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                                }
                                ...
                                sourcingStringBuilder2.setSpan(new ForegroundColorSpan(Color.parseColor("#" + sourceColor)), 0,
                                        ...
                                        Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                                //benefit_seperator_line 색상 지정
                                sourcingStringBuilder2.setSpan(new ForegroundColorSpan(Color.parseColor("#33111111")),
                                        tempTvLiveBanner.rSource.text.length() + 1,
                                        tempTvLiveBanner.rSource.text.length() + 3,
                                        Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                            }
                            catch (IndexOutOfBoundsException e) {
                                Ln.e(e.getMessage());
                        ...
                        if (tempTvLiveBanner.rAllBenefit.size() > 0) {
                            for (ImageBadge badge : tempTvLiveBanner.rAllBenefit) {
                                ...
                                /*
                                뱃지 색상 지정할 때에 뱃지가 잘린 상태라면 뱃지 비교에서 같지 않게되어 색상을 칠하지 않음.
                                이를 아래와 같이 수정할 수 있으나 추가 문제 있을 수 있어 보류
                                if (index < 0) {
                                    String[] strExtraBadges = badge.text.split(" ");
                                    for(String strTemp : strExtraBadges) {
                                        if (strText.indexOf(strTemp) > -1) {
                                            index = strText.indexOf((strTemp));
                                            badge.text = strTemp;
                                            break;
                                        }
                                    }
                                ...
                                if (index >= 0 && !TextUtils.isEmpty(badge.type)) {
                                    try {
                                        sourcingStringBuilder2.setSpan(new ForegroundColorSpan(Color.parseColor("#" + badge.type)),
                                                ...
                                                Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                                        //benefit_seperator_dot 색상 적용
                                        sourcingStringBuilder2.setSpan(new ForegroundColorSpan(Color.parseColor("#4c111111")),
