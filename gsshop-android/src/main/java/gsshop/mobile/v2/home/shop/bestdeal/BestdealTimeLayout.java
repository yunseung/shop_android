 ...
 *
 */
public class BestdealTimeLayout extends LinearLayout {
     ...
     * 생방송/데이타방송 구분
     */
    public enum TimerBroadType {
        live, data
    }
    TimerBroadType timerBroadType = TimerBroadType.live;

    private static final String TAG = "TvShopTimeLayout";

    private TextView mTxtRemainTimeLive;
    private TextView mTxtRemainTimeData;
    private TextView mTxtRemainTimeLiveP;
    private TextView mTxtRemainTimeDataP;

    public String endDate;

    // 방송종료 시점을 알기 위한 listener
    private OnTvLiveFinishedListener mOnTvLiveFinishedListener;
    private OnDataLiveFinishedListener mOnDataLiveFinishedListener;

    public BestdealTimeLayout(Context context, AttributeSet attrs, int defStyle) {
        this(context, attrs);
        this.mContext = context;
    }

    public BestdealTimeLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.mContext = context;
    }

    public BestdealTimeLayout(Context context) {
        this(context, null);
        this.mContext = context;
    }
    ...
    protected void onFinishInflate() {
        ...
        mTxtRemainTimeData = (TextView) findViewById(R.id.txt_remain_time_data_play);
        mTxtRemainTimeLiveP = (TextView) findViewById(R.id.txt_remain_time_live_pause);
        mTxtRemainTimeDataP = (TextView) findViewById(R.id.txt_remain_time_data_pause);
    }
     ...
     * @param endTime endTime
     */
    public void updateTvLiveTime(BroadType broadType, String startTime, String endTime) {
        if("".equals(startTime) && "".equals(endTime)){
            if (mTxtRemainTimeLive != null) {
                //mTxtRemainTimeLive.setText(R.string.home_tv_live_view_ad_text);
            }
            if (mTxtRemainTimeData != null) {
                //mTxtRemainTimeData.setText(R.string.home_tv_live_view_ad_text);
            }
            return;
        }

        if (broadType == BroadType.live) {
            timerBroadType = TimerBroadType.live;
        } if (broadType == BroadType.data) {
            timerBroadType = TimerBroadType.data;
        }
        ...
        ((Activity)mContext).runOnUiThread(() -> {
            try{
                ...
                if(timeRemaining.isAfterTime()){
                    // 방송 중
                    if (timerBroadType == TimerBroadType.live) {
                        if (mTxtRemainTimeLive != null) {
                    ...
                    if (timerBroadType == TimerBroadType.data) {
                        if (mTxtRemainTimeData != null) {
                            mTxtRemainTimeData.setText(timeRemaining.getDisplayTime(TimeRemaining.DisplayType.HOUR));
                        }
                        if (mTxtRemainTimeDataP != null) {
                            mTxtRemainTimeDataP.setText(timeRemaining.getDisplayTime(TimeRemaining.DisplayType.HOUR));
                        }
                    }
                }else{
                    // 방송 종료 전후
                    ...
                    if (timerBroadType == TimerBroadType.data) {
                        if (mTxtRemainTimeData != null) {
                            mTxtRemainTimeData.setText(R.string.home_best_deal_view_close_text);
                        }
                        if (mTxtRemainTimeDataP != null) {
                            mTxtRemainTimeDataP.setText(R.string.home_best_deal_view_close_text);
                        }
                        if (mOnDataLiveFinishedListener != null) {
                            stopTimer();
                            //방송 재호출시 부하를 줄이기 위해 4초에서 14초 사이의 랜덤값으로 api를 호출한다.
                            int randomInteger = (new Random().nextInt(10) + 4 )* 1000;
                            new Handler().postDelayed(new Runnable() {
                                @Override
                                public void run() {
                                    mOnDataLiveFinishedListener.onDataLiveFinished();
                                }
                            }, randomInteger);
                        }
                    }
    ...
    /**
     * 데이타방송종료 콜백용 인터페이스를 세팅한다.
     *
     * @param listener OnDataLiveFinishedListener
     */
    public void setOnDataLiveFinishedListener(OnDataLiveFinishedListener listener) {
        mOnDataLiveFinishedListener = listener;
    }
     ...
     * 데이타방송종료 콜백용 인터페이스
     */
    public interface OnDataLiveFinishedListener {
