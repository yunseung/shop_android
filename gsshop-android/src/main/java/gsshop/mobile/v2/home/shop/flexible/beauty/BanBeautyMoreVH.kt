...
class BanBeautyMoreVH(itemView: View) : BaseViewHolder(itemView) {
...
//        viewMore?.setViewResources(R.layout.view_holder_ban_read_more_wine)
        ...
        viewMore?.setSeperator(false)

        itemView.root_view.setOnClickListener {
            if (!TextUtils.isEmpty(item.tabSeq)) {
                EventBus.getDefault().post(Events.BeautyShopEvent.BeautyMoveTabEvent(item.tabSeq, item.linkUrl))
            }
        }
    }
}