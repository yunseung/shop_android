 ...
 */
@SuppressLint("NewApi")
public class FlexibleBannerImageViewHolder extends BaseViewHolder {
	 ...
	 * @param itemView
	 */
	public FlexibleBannerImageViewHolder(View itemView) {
		super(itemView);
...
//		main_img.setContentDescription(item.productName);
		...
		else if(!TextUtils.isEmpty(item.gsAccessibilityVariable)) {
			main_img.setContentDescription(item.gsAccessibilityVariable);
		}
		else {
			main_img.setContentDescription(context.getString(R.string.description_image));
		}
