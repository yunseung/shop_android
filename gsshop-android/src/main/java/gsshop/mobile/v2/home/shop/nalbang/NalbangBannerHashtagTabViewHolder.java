 ...
 *
 */
public class NalbangBannerHashtagTabViewHolder extends BaseViewHolder {
     ...
     * @param itemView
     */
    public NalbangBannerHashtagTabViewHolder(View itemView) {
        super(itemView);
        ...
        BOARDER_COLOR_SELECTED = MainApplication.getAppContext().getResources().getColor(R.color.nalbang_shop_hashtab_item_border_selected);
    }
                ...
                new ViewTreeObserver.OnPreDrawListener() {
                    ...
                    public boolean onPreDraw() {
                        if (selectedView.getWidth() > 0) {
                            ...
                            int columnPos = 0;
                            if (info.tabIndex >= 0) {
                                rowPos = info.tabIndex / COLUMN_RES_IDS.length;
                                columnPos = info.tabIndex % COLUMN_RES_IDS.length;
                            }
                                ...
                                for (int i = 0; i < hashtagTable.getChildCount(); i++) {
                                    ...
                                    for (int j = 0; j < row.getChildCount(); j++) {
                                        ...
                                        if (rect.contains(x, y)) {
                                            // 동일 tab이 선택되면 무시.
                                            if(tab !=info.tabIndex) {
                                                EventBus.getDefault().post(new Events.FlexibleEvent.NalbangHashTagTabUpdate(tab));
