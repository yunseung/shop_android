...
class PositionedCropTransformation : BitmapTransformation {
    ...
    constructor(context: Context?, @FloatRange(from = 0.0, to = 1.0) xPercentage: Float, @FloatRange(from = 0.0, to = 1.0) yPercentage: Float) : super(context) {
    ...
    override fun transform(pool: BitmapPool, toTransform: Bitmap, outWidth: Int, outHeight: Int): Bitmap? {
        val toReuse = pool[outWidth, outHeight, if (toTransform.config != null) toTransform.config else Bitmap.Config.ARGB_8888]
    ...
    companion object {
         ...
         */
        private fun crop(recycled: Bitmap?, toCrop: Bitmap?, width: Int, height: Int, xPercentage: Float, yPercentage: Float): Bitmap? {
            if (toCrop == null) {
                return null
